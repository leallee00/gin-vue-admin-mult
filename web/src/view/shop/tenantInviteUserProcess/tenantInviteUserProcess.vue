<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="处理进度，字典定义" prop="process" width="120">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateTenantInviteUserProcessFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteTenantInviteUserProcess,
  deleteTenantInviteUserProcessByIds,
  updateTenantInviteUserProcess,
  findTenantInviteUserProcess,
  getTenantInviteUserProcessList
...
const getTableData = async() => {
  const table = await getTenantInviteUserProcessList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteTenantInviteUserProcessFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteTenantInviteUserProcessByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateTenantInviteUserProcessFunc = async(row) => {
    const res = await findTenantInviteUserProcess({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteTenantInviteUserProcessFunc = async (row) => {
    const res = await deleteTenantInviteUserProcess({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
