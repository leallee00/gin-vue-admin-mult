<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="申请id" prop="applyId">
             ...
             <el-input v-model.number="searchInfo.applyId" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="申请id" prop="applyId" width="120" />
        <el-table-column align="left" label="操作原因" prop="desc" width="120" />
        <el-table-column align="left" label="操作者id" prop="optUserId" width="120" />
        <el-table-column align="left" label="process字段" prop="process" width="120">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateUserApplyJoinTenantProcessFlowFunc(scope.row)">变更</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="申请id:"  prop="applyId" >
              <el-input v-model.number="formData.applyId" :clearable="true" placeholder="请输入申请id" />
            </el-form-item>
            <el-form-item label="操作原因:"  prop="desc" >
...
import {
  ...
  deleteUserApplyJoinTenantProcessFlow,
  deleteUserApplyJoinTenantProcessFlowByIds,
  updateUserApplyJoinTenantProcessFlow,
  findUserApplyJoinTenantProcessFlow,
  getUserApplyJoinTenantProcessFlowList
...
defineOptions({
    name: 'UserApplyJoinTenantProcessFlow'
})
...
const getTableData = async() => {
  const table = await getUserApplyJoinTenantProcessFlowList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteUserApplyJoinTenantProcessFlowFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserApplyJoinTenantProcessFlowByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserApplyJoinTenantProcessFlowFunc = async(row) => {
    const res = await findUserApplyJoinTenantProcessFlow({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserApplyJoinTenantProcessFlowFunc = async (row) => {
    const res = await deleteUserApplyJoinTenantProcessFlow({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
